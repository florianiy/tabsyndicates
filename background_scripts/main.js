...
//
//
// load potentially saved data
//       and
// setup save
// does not seem to work
// async function haha() {
//   const gg = store.get("groups");
//   if (Object.keys(gg).length) groups = gg;
//   Object.keys(groups).map((groupid) => {
//     const it = groups[groupid];
//     CreateGroup(it.name, it.color);
//     CreateSyndicateForum({ index: it.tabs[0] }, groupid);
//     it.tabs.forEach((__id) => UpdateGroupForTab(__id, groupid));
//   });
//   setInterval(() => {
//     if (Object.keys(groups).length) store.set({ groups });
//   }, 2000);
// }
//
// store.get("groups").then((it) => {
//   groups = it.groups || {};

//   Object.keys(groups).forEach((groupid) => {
//     const ji = groups[groupid];

//     CreateGroup(ji.name, ji.color);
//     CreateSyndicateForum({ index: ji.tabs[0] }, groupid);
//   });

//   save_enabled = true;
// });

// asta va fi locul in care pun cate 2 '/' ca sa fortez refresh
